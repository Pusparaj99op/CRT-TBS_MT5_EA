...
void CCRTAnalyzer::AnalyzeFVG(int index, ENUM_TIMEFRAMES tf)
  {
   ...
   // CopyHigh/Low with index=1, count=3 fills arrays as:
   //   [0] = bar[1] = newest closed bar  (right side of FVG window)
   //   [1] = bar[2] = middle bar
   //   [2] = bar[3] = oldest bar         (left side of FVG window)
   double left_high  = h[2];   // oldest bar high
   double left_low   = l[2];   // oldest bar low
   double right_high = h[0];   // newest bar high
   double right_low  = l[0];   // newest bar low
   datetime event_time = t[1]; // middle candle marks the gap time

   double min_gap = m_fvg_min_pips * m_pip_value;

   // Bullish FVG: newest bar's low is above oldest bar's high (gap up — price left a void below)
   if(right_low > left_high && (right_low - left_high) >= min_gap)
     {
      m_last_bull_fvg.active = true;
      m_last_bull_fvg.high_price = right_low;
      m_last_bull_fvg.low_price = left_high;
      m_last_bull_fvg.time = event_time;
      if(m_draw_fvg)
        {
         string name = "CRT_FVG_BULL_" + TimeToString(event_time);
         DrawFVGBox(name, event_time, right_low, left_high, clrPaleGreen);
        }
     }

   // Bearish FVG: newest bar's high is below oldest bar's low (gap down — price left a void above)
   if(right_high < left_low && (left_low - right_high) >= min_gap)
     {
      m_last_bear_fvg.active = true;
      m_last_bear_fvg.high_price = left_low;
      m_last_bear_fvg.low_price = right_high;
      m_last_bear_fvg.time = event_time;
      if(m_draw_fvg)
        {
         string name = "CRT_FVG_BEAR_" + TimeToString(event_time);
         DrawFVGBox(name, event_time, left_low, right_high, clrMistyRose);
        }
     }
  }
...
void CCRTAnalyzer::AnalyzeOB(int index, ENUM_TIMEFRAMES tf)
  {
   ...
   // Bullish OB: last bearish candle before strong bullish impulse that closes above its high
   if(prevBear && currBull && currBody > 10 * m_pip_value && c[1] > h[0])
     {
   ...
   // Bearish OB: last bullish candle before strong bearish impulse that closes below its low
   if(prevBull && currBear && currBody > 10 * m_pip_value && c[1] < l[0])
     {
   ...
   if(!m_acc_range.active)
     {
      ...
      if(CopyHigh(Symbol(), tf, index, m_min_acc_candles, highs) == m_min_acc_candles &&
         ...
         CopyClose(Symbol(), tf, index, m_min_acc_candles, closes) == m_min_acc_candles)
        {
         ...
         // 200 pips = $20 for XAUUSD (m_pip_value=0.1), suitable for M5 Gold consolidations
         double max_acc_range = 200.0 * m_pip_value;
