...
bool CRiskManager::IsMarginSufficient(double lot_size)
  {
   ...
   if(!OrderCalcMargin(ORDER_TYPE_BUY, Symbol(), lot_size,
     ...
     {
      // OrderCalcMargin can fail during Strategy Tester initialisation or with
      // incomplete tick data. Fall back to a simple free-margin percentage check
      // rather than silently blocking every trade.
      if(g_logger != NULL)
         g_logger.LogSimple("Warning: OrderCalcMargin failed. Using free-margin fallback.");
      return (AccountInfoDouble(ACCOUNT_MARGIN_FREE) > AccountInfoDouble(ACCOUNT_BALANCE) * 0.05);
     }
